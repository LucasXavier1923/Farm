...
$raw = (& $cli run-tool screenshot-scene-view $project --input-file '.codex\screenshot-scene-view.json' --raw 2>&1) -join "`n"
$exitCode = $LASTEXITCODE
$ErrorActionPreference = $previousPreference
if ($exitCode -ne 0) { throw $raw }
$jsonStart = $raw.IndexOf('{')
$jsonEnd = $raw.LastIndexOf('}')
if ($jsonStart -lt 0 -or $jsonEnd -lt $jsonStart) { throw 'Resposta JSON ausente na captura da Scene View' }
...
if ($null -eq $image -or [string]::IsNullOrWhiteSpace($image.data)) { throw 'Imagem ausente na captura da Scene View' }
