...
namespace FarmPrototype.Farming
{
    ...
    public sealed class FarmSettingsMenu : MonoBehaviour
    {
        ...
        public void Initialize(FarmHudController owner)
        {
            ...
            if (canvas == null) throw new InvalidOperationException("Canvas da fazenda ausente para configura\u00E7\u00F5es.");
        ...
        private void Update()
        {
            ...
            if (open && (hud.IsInventoryOpen || hud.IsStorageOpen || hud.IsJournalOpen || hud.IsSleepConfirmationOpen || hud.IsDailyOrdersOpen || hud.IsMasteryOpen || hud.IsCraftingOpen)) SetOpen(false);
        }
        ...
        private void RefreshLabels()
        {
            ...
            zoomValue.text = FarmLocalization.Format("settings.zoom.value", "{0:0.0} units", FarmSettings.ZoomStep);
            invertValue.text = FarmSettings.InvertVertical
                ? FarmLocalization.Get("settings.invert.enabled", "ENABLED")
                : FarmLocalization.Get("settings.invert.disabled", "DISABLED");
        ...
        {
            var button = CreateButton("OpenSettings", root, "settings.launch", new Vector2(1f, 1f), new Vector2(-18f, -282f), new Vector2(210f, 44f), new Vector2(1f, 1f));
        ...
        private void CreateWindow(Transform root)
        {
            ...
            CreateText("Title", panel.transform, "settings.title", 30, FontStyle.Bold, AccentColor, new Vector2(30f, -22f), new Vector2(500f, 42f));
            CreateText("Subtitle", panel.transform, "settings.subtitle", 16, FontStyle.Normal, new Color(0.78f, 0.84f, 0.74f), new Vector2(30f, -66f), new Vector2(600f, 28f));
            var close = CreateButton("CloseSettings", panel.transform, "ui.close.esc", new Vector2(1f, 1f), new Vector2(-28f, -24f), new Vector2(160f, 44f), new Vector2(1f, 1f));
            close.onClick.AddListener(() => SetOpen(false));
            CreateSettingRow(panel.transform, "settings.camera_sensitivity", "settings.camera_sensitivity.hint", -122f, out sensitivityValue, () => AdjustSensitivity(-0.02f), () => AdjustSensitivity(0.02f));
            CreateSettingRow(panel.transform, "settings.zoom_speed", "settings.zoom_speed.hint", -238f, out zoomValue, () => AdjustZoom(-0.5f), () => AdjustZoom(0.5f));
            CreateText("InvertTitle", panel.transform, "settings.invert.title", 18, FontStyle.Bold, Color.white, new Vector2(38f, -354f), new Vector2(320f, 28f));
            CreateText("InvertHint", panel.transform, "settings.invert.hint", 14, FontStyle.Normal, new Color(0.72f, 0.78f, 0.70f), new Vector2(38f, -385f), new Vector2(430f, 24f));
            ...
            var reset = CreateButton("ResetSettings", panel.transform, "settings.reset", new Vector2(0.5f, 0f), new Vector2(0f, 28f), new Vector2(260f, 48f), new Vector2(0.5f, 0f));
        ...
        private Text CreateText(string name, Transform parent, string content, int size, FontStyle style, Color color, Vector2 position, Vector2 dimensions, TextAnchor alignment = TextAnchor.UpperLeft)
        {
            ...
            var text = item.GetComponent<Text>(); text.font = font; text.text = FarmLocalization.Get(content, content); text.fontSize = size; text.fontStyle = style; text.color = color; text.alignment = alignment; text.horizontalOverflow = HorizontalWrapMode.Wrap; text.verticalOverflow = VerticalWrapMode.Overflow;
            return text;
        }
