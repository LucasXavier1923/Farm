...
This creates clear cultivation, harvesting/production, and commerce roles in a four-player farm while keeping every normal activity available to a solo tester. A future Supabase profile must store this per player rather than using the current shared prototype state; the host `Progression` intent already provides the transition boundary.
